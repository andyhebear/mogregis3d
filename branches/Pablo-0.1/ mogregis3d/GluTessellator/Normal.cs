...
namespace Mogre.Utils.GluTesselator
{
	...
	class Normal
	{
		...
		internal static bool SLANTED_SWEEP = false;
		internal static double S_UNIT_X; /* Pre-normalized */
		internal static double S_UNIT_Y;
		private const bool TRUE_PROJECT = false;
		...
		internal static int LongAxis(double[] v)
		{
			int i = 0;
			
			if (System.Math.Abs(v[1]) > System.Math.Abs(v[0]))
			{
				i = 1;
			}
			if (System.Math.Abs(v[2]) > System.Math.Abs(v[i]))
			{
				i = 2;
			}
			return i;
