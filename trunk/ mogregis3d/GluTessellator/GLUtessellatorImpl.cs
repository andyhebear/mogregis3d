...
namespace Mogre.Utils.GluTesselator
{
	...
	{
		public const int TESS_MAX_CACHE = 100;
		
		private int state; /* what begin/end calls have we seen? */
		
		private GLUhalfEdge lastEdge; /* lastEdge->Org is the most recent vertex */
		internal GLUmesh mesh; /* stores the input contours, and eventually
		the tessellation itself */
		
		/// <summary> state needed for projecting onto the sweep plane **</summary>
		
		internal double[] normal = new double[3]; /* user-specified normal (if provided) */
		internal double[] sUnit = new double[3]; /* unit vector in s-direction (debugging) */
		internal double[] tUnit = new double[3]; /* unit vector in t-direction (debugging) */
		
		/// <summary> state needed for the line sweep **</summary>
		
		private double relTolerance; /* tolerance for merging features */
		internal int windingRule; /* rule for determining polygon interior */
		internal bool fatalError; /* fatal error: needed combine callback */
		
		internal Dict dict; /* edge dictionary for sweep line */
		internal PriorityQ pq; /* priority queue of vertex events */
		internal GLUvertex event_Renamed; /* current sweep event being processed */
		
		/// <summary> state needed for rendering callbacks (see render.c) **</summary>
		
		internal bool flagBoundary; /* mark boundary edges (use EdgeFlag) */
		internal bool boundaryOnly; /* Extract contours, not triangles */
		internal GLUface lonelyTriList;
		/* list of triangles which could not be rendered as strips or fans */
		
		
		
		/// <summary> state needed to cache single-contour polygons for renderCache() </summary>
		
		private bool flushCacheOnNextVertex; /* empty cache on next vertex() call */
		internal int cacheCount; /* number of cached vertices */
		internal CachedVertex[] cache = new CachedVertex[TESS_MAX_CACHE]; /* the vertex data */
		
		/// <summary> rendering callbacks that also pass polygon data  **</summary>
		private System.Object polygonData; /* client data for current polygon */
		...
		private const double GLU_TESS_DEFAULT_TOLERANCE = 0.0;
				...
				*/
				if (state < newState)
				{
					if (state == TessState.T_DORMANT)
					{
						...
						gluTessBeginPolygon((System.Object) null);
					}
		...
		public virtual void  gluDeleteTess()
		{
			requireState(TessState.T_DORMANT);
		}
		
		public virtual void  gluTessProperty(int which, double value_Renamed)
		{
			switch (which)
			{
				
				case GLU.GLU_TESS_TOLERANCE: 
					if (value_Renamed < 0.0 || value_Renamed > 1.0)
						break;
					relTolerance = value_Renamed;
					return ;
				...
				case GLU.GLU_TESS_WINDING_RULE: 
					...
					int windingRule = (int) value_Renamed;
					if (windingRule != value_Renamed)
					...
					goto case GLU.GLU_TESS_BOUNDARY_ONLY;
				
				
				case GLU.GLU_TESS_BOUNDARY_ONLY: 
					boundaryOnly = (value_Renamed != 0);
					return ;
		...
		public virtual void  gluGetTessProperty(int which, double[] value_Renamed, int value_offset)
		{
			switch (which)
			{
				...
				case GLU.GLU_TESS_TOLERANCE: 
					...
					value_Renamed[value_offset] = relTolerance;
					break;
				
				case GLU.GLU_TESS_WINDING_RULE: 
					//assert(windingRule == GLU.GLU_TESS_WINDING_ODD || windingRule == GLU.GLU_TESS_WINDING_NONZERO || windingRule == GLU.GLU_TESS_WINDING_POSITIVE || windingRule == GLU.GLU_TESS_WINDING_NEGATIVE || windingRule == GLU.GLU_TESS_WINDING_ABS_GEQ_TWO);
					value_Renamed[value_offset] = windingRule;
					break;
				
				case GLU.GLU_TESS_BOUNDARY_ONLY: 
					//assert(boundaryOnly == true || boundaryOnly == false);
					value_Renamed[value_offset] = boundaryOnly?1:0;
					break;
				
				default: 
					value_Renamed[value_offset] = 0.0;
					callErrorOrErrorData(GLU.GLU_INVALID_ENUM);
					break;
		...
		public virtual void  gluTessNormal(double x, double y, double z)
		{
			normal[0] = x;
			normal[1] = y;
			normal[2] = z;
		...
		public virtual void  gluTessEndContour()
		{
			requireState(TessState.T_IN_CONTOUR);
			state = TessState.T_IN_POLYGON;
		}
		...
		public virtual void  gluBeginPolygon()
		{
			gluTessBeginPolygon((System.Object) null);
			gluTessBeginContour();
		}
		...
		/*ARGSUSED*/
		public virtual void  gluNextContour(int type)
		{
			gluTessEndContour();
			gluTessBeginContour();
		}
		...
		public virtual void  gluEndPolygon()
		{
			gluTessEndContour();
			gluTessEndPolygon();
		}
		
		internal virtual void  callBeginOrBeginData(int a)
		{
		...
		internal virtual void  callVertexOrVertexData(System.Object a)
		{
		...
		internal virtual void  callEdgeFlagOrEdgeFlagData(bool a)
		{
		...
		internal virtual void  callEndOrEndData()
		{
		...
		internal virtual void  callCombineOrCombineData(double[] coords, System.Object[] vertexData, float[] weights, System.Object[] outData)
		{
		...
		internal virtual void  callErrorOrErrorData(int a)
		{
