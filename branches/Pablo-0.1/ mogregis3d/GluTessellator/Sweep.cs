...
namespace Mogre.Utils.GluTesselator
{
	...
	class Sweep
	{
		...
		{
			public virtual bool leq(System.Object key1, System.Object key2)
			{
		...
		*/
		...
		internal static bool EdgeLeq(GLUtessellatorImpl tess, ActiveRegion reg1, ActiveRegion reg2)
		/*
		...
		*/
		{
			GLUvertex event_Renamed = tess.event_Renamed;
			GLUhalfEdge e1, e2;
			double t1, t2;
			...
			if (e1.Sym.Org == event_Renamed)
			{
				if (e2.Sym.Org == event_Renamed)
				{
				...
				return Geom.EdgeSign(e2.Sym.Org, event_Renamed, e2.Org) <= 0;
			}
			if (e2.Sym.Org == event_Renamed)
			{
				return Geom.EdgeSign(e1.Sym.Org, event_Renamed, e1.Org) >= 0;
			}
			
			/* General case - compute signed distance *from* e1, e2 to event */
			t1 = Geom.EdgeEval(e1.Sym.Org, event_Renamed, e1.Org);
			t2 = Geom.EdgeEval(e2.Sym.Org, event_Renamed, e2.Org);
			return (t1 >= t2);
		}
		
		
		internal static void  DeleteRegion(GLUtessellatorImpl tess, ActiveRegion reg)
		{
			if (reg.fixUpperEdge)
			{
		...
		internal static bool FixUpperEdge(ActiveRegion reg, GLUhalfEdge newEdge)
		/*
		...
		*/
		...
		internal static ActiveRegion TopLeftRegion(ActiveRegion reg)
		{
		...
		internal static ActiveRegion TopRightRegion(ActiveRegion reg)
		{
		...
		internal static ActiveRegion AddRegionBelow(GLUtessellatorImpl tess, ActiveRegion regAbove, GLUhalfEdge eNewUp)
		/*
		...
		*/
		...
		internal static bool IsWindingInside(GLUtessellatorImpl tess, int n)
		{
		...
		internal static void  ComputeWinding(GLUtessellatorImpl tess, ActiveRegion reg)
		{
		...
		internal static void  FinishRegion(GLUtessellatorImpl tess, ActiveRegion reg)
		/*
		...
		*/
		...
		internal static GLUhalfEdge FinishLeftRegions(GLUtessellatorImpl tess, ActiveRegion regFirst, ActiveRegion regLast)
		/*
		...
		*/
		...
		internal static void  AddRightEdges(GLUtessellatorImpl tess, ActiveRegion regUp, GLUhalfEdge eFirst, GLUhalfEdge eLast, GLUhalfEdge eTopLeft, bool cleanUp)
		/*
		...
		*/
		...
		internal static void  CallCombine(GLUtessellatorImpl tess, GLUvertex isect, System.Object[] data, float[] weights, bool needed)
		{
			...
			System.Object[] outData = new System.Object[1];
		...
		internal static void  SpliceMergeVertices(GLUtessellatorImpl tess, GLUhalfEdge e1, GLUhalfEdge e2)
		/*
		* Two vertices with idential coordinates are combined into one.
		* e1.Org is kept, while e2.Org is discarded.
		*/
		{
			System.Object[] data = new System.Object[4];
		...
		internal static void  VertexWeights(GLUvertex isect, GLUvertex org, GLUvertex dst, float[] weights)
		/*
		...
		*/
		...
		internal static void  GetIntersectData(GLUtessellatorImpl tess, GLUvertex isect, GLUvertex orgUp, GLUvertex dstUp, GLUvertex orgLo, GLUvertex dstLo)
		/*
		...
		*/
		{
			...
			Array.Copy(weights1, 0, weights, 0, 2);
			Array.Copy(weights2, 0, weights, 2, 2);
			
			CallCombine(tess, isect, data, weights, true);
		}
		
		internal static bool CheckForRightSplice(GLUtessellatorImpl tess, ActiveRegion regUp)
		/*
		...
		*/
		{
			...
			{
				if (Geom.EdgeSign(eLo.Sym.Org, eUp.Org, eLo.Org) > 0)
					return false;
			...
			{
				if (Geom.EdgeSign(eUp.Sym.Org, eLo.Org, eUp.Org) < 0)
					return false;
		...
		internal static bool CheckForLeftSplice(GLUtessellatorImpl tess, ActiveRegion regUp)
		/*
		* Check the upper and lower edge of "regUp", to make sure that the
		...
		*/
		{
			...
			{
				if (Geom.EdgeSign(eUp.Sym.Org, eLo.Sym.Org, eUp.Org) < 0)
					return false;
			...
			{
				if (Geom.EdgeSign(eLo.Sym.Org, eUp.Sym.Org, eLo.Org) > 0)
					return false;
		...
		internal static bool CheckForIntersect(GLUtessellatorImpl tess, ActiveRegion regUp)
		/*
		...
		*/
		{
			...
			//assert(Geom.EdgeSign(dstUp, tess.event_Renamed, orgUp) <= 0);
			//assert(Geom.EdgeSign(dstLo, tess.event_Renamed, orgLo) >= 0);
			//assert(orgUp != tess.event_Renamed && orgLo != tess.event_Renamed);
			//assert(!regUp.fixUpperEdge && !regLo.fixUpperEdge);
			
			if (orgUp == orgLo)
				return false; /* right endpoints are the same */
			
			tMinUp = System.Math.Min(orgUp.t, dstUp.t);
			tMaxLo = System.Math.Max(orgLo.t, dstLo.t);
			if (tMinUp > tMaxLo)
			...
			//assert(System.Math.Min(orgUp.t, dstUp.t) <= isect.t);
			//assert(isect.t <= System.Math.Max(orgLo.t, dstLo.t));
			//assert(System.Math.Min(dstLo.s, dstUp.s) <= isect.s);
			//assert(isect.s <= System.Math.Max(orgLo.s, orgUp.s));
			
			if (Geom.VertLeq(isect, tess.event_Renamed))
			{
				...
				*/
				isect.s = tess.event_Renamed.s;
				isect.t = tess.event_Renamed.t;
			}
			...
			*/
			...
			if ((!Geom.VertEq(dstUp, tess.event_Renamed) && Geom.EdgeSign(dstUp, tess.event_Renamed, isect) >= 0) || (!Geom.VertEq(dstLo, tess.event_Renamed) && Geom.EdgeSign(dstLo, tess.event_Renamed, isect) <= 0))
			{
				/* Very unusual -- the new upper or lower edge would pass on the
				* wrong side of the sweep event, or through it.  This can happen
				* due to very small numerical errors in the intersection calculation.
				*/
				if (dstLo == tess.event_Renamed)
				{
				...
				*/
				if (Geom.EdgeSign(dstUp, tess.event_Renamed, isect) >= 0)
				{
					RegionAbove(regUp).dirty = regUp.dirty = true;
					if (Mesh.__gl_meshSplitEdge(eUp.Sym) == null)
						throw new System.SystemException();
					eUp.Org.s = tess.event_Renamed.s;
					eUp.Org.t = tess.event_Renamed.t;
				}
				if (Geom.EdgeSign(dstLo, tess.event_Renamed, isect) <= 0)
				{
					regUp.dirty = regLo.dirty = true;
					if (Mesh.__gl_meshSplitEdge(eLo.Sym) == null)
						throw new System.SystemException();
					eLo.Org.s = tess.event_Renamed.s;
					eLo.Org.t = tess.event_Renamed.t;
				}
			...
			*/
			...
			if (eUp.Org.pqHandle == int.MaxValue)
			{
		...
		internal static void  WalkDirtyRegions(GLUtessellatorImpl tess, ActiveRegion regUp)
		/*
		...
		*/
		{
			...
			for (; ; )
			{
				...
				if (eUp.Org != eLo.Org)
				{
					if (eUp.Sym.Org != eLo.Sym.Org && !regUp.fixUpperEdge && !regLo.fixUpperEdge && (eUp.Sym.Org == tess.event_Renamed || eLo.Sym.Org == tess.event_Renamed))
					{
		...
		internal static void  ConnectRightVertex(GLUtessellatorImpl tess, ActiveRegion regUp, GLUhalfEdge eBottomLeft)
		/*
		...
		*/
		{
			...
			*/
			if (Geom.VertEq(eUp.Org, tess.event_Renamed))
			{
			...
			if (Geom.VertEq(eLo.Org, tess.event_Renamed))
			{
		...
		*/
		private const bool TOLERANCE_NONZERO = false;
		
		internal static void  ConnectLeftDegenerate(GLUtessellatorImpl tess, ActiveRegion regUp, GLUvertex vEvent)
		/*
		...
		*/
		...
		internal static void  ConnectLeftVertex(GLUtessellatorImpl tess, GLUvertex vEvent)
		/*
		...
		*/
		...
		internal static void  SweepEvent(GLUtessellatorImpl tess, GLUvertex vEvent)
		/*
		...
		*/
		{
			...
			tess.event_Renamed = vEvent; /* for access in EdgeLeq() */
			DebugEvent(tess);
		...
		*/
		//UPGRADE_NOTE: Final was removed from the declaration of 'SENTINEL_COORD '. 'ms-help://MS.VSCC.2003/commoner/redir/redirect.htm?keyword="jlca1003_3"'
		private static readonly double SENTINEL_COORD = (4.0 * GLU.GLU_TESS_MAX_COORD);
		
		internal static void  AddSentinel(GLUtessellatorImpl tess, double t)
		/*
		...
		*/
		{
			...
			e.Sym.Org.t = t;
			tess.event_Renamed = e.Sym.Org; /* initialize it */
		...
		internal static void  InitEdgeDict(GLUtessellatorImpl tess)
		/*
		...
		*/
		{
			...
			tess.dict = Dict.dictNewDict(tess, new AnonymousClassDictLeq(tess));
			if (tess.dict == null)
				throw new System.SystemException();
		...
		internal static void  DoneEdgeDict(GLUtessellatorImpl tess)
		{
			ActiveRegion reg;
			int fixedEdges = 0;
		...
		internal static void  RemoveDegenerateEdges(GLUtessellatorImpl tess)
		/*
		...
		*/
			...
			for (e = eHead.next; e != eHead; e = eNext)
			{
				...
				if (Geom.VertEq(e.Org, e.Sym.Org) && e.Lnext.Lnext != e)
				{
					...
					if (!Mesh.__gl_meshDelete(e))
						throw new System.SystemException(); /* e is a self-loop */
					e = eLnext;
					eLnext = e.Lnext;
				}
				if (eLnext.Lnext == e)
		...
		internal static bool InitPriorityQ(GLUtessellatorImpl tess)
		/*
		...
		*/
		{
			...
			pq = tess.pq = PriorityQ.pqNewPriorityQ(new AnonymousClassLeq());
			if (pq == null)
				return false;
		...
		internal static void  DonePriorityQ(GLUtessellatorImpl tess)
		{
			tess.pq.pqDeletePriorityQ(); /* __gl_pqSortDeletePriorityQ */
		}
		
		
		internal static bool RemoveDegenerateFaces(GLUmesh mesh)
		/*
		...
		*/
		...
		public static bool __gl_computeInterior(GLUtessellatorImpl tess)
		/*
		...
		*/
		{
			...
			*/
			...
			/* __GL_DICTLISTKEY */ /* __GL_DICTLISTMIN */
			tess.event_Renamed = ((ActiveRegion) Dict.dictKey(Dict.dictMin(tess.dict))).eUp.Org;
			DebugEvent(tess);
			DoneEdgeDict(tess);
			DonePriorityQ(tess);
